...
class core_processing:
    ...
    def callback3(self,data):
        ...
        global front_test
        global rear_test
        global headingAngle_count_max
        global headingAngle_count
        global headingAngle_ref
        global lidarPreviousError
        filter_weight_IMU = 0.8
        ...
        #only start if the lidar has been initialized
        if(initial_lidar):        
            fr_dist = (leftData['fr_dist'] + rightData['fr_dist']) / 2
            totalCornerNum = leftData['cornerNum'] + rightData['cornerNum']
            turn_dist = (leftData['turn_dist'] + rightData['turn_dist']) / 2
            print("turn_dist: " + str(turn_dist) + "  " + "headingAngle_ref: " + str(headingAngle_ref))

            #if the hovercraft is too close to the front wall, then stop
            if(fr_dist< 20):
                print("Stop!!!!")
                while(True):
                    front_motor = 0
            ...
            if(turn_dist>120):
                ...
                if((abs(rightData['line_dist']) < 150) and (abs(leftData['line_dist']) < 150)):
                    dist_diff = rightData['line_dist']-leftData['line_dist']

                    dist_diff = 0.9*lidarPreviousError + 0.1*dist_diff
                else:
                    dist_diff = 0
                    dist_diff = 0.9*lidarPreviousError + 0.1*dist_diff
                    
                
                targetAngle= headingAngle_ref - lidarKp*dist_diff
         
                front_motor = front_test
                rear_motor = rear_test
            
                angle_feedback = steerPidControl(targetAngle, angle_now, gyro_now)
                
                    
                servo_angle = newMap(angle_feedback,-90,90,180,0)
                print("targetAngle: " + str(targetAngle) + "  " + "angle_now: " + str(angle_now))

                if((((headingAngle_ref>89) and (headingAngle_ref<91)) or  ((headingAngle_ref>-91) and (headingAngle_ref<-89)))and (turn_dist<200)):
                    rear_motor = rear_test-4
                else:
                    rear_motor = rear_test+2
            
            
            
            #if the hovercraft is too close to the front wall then turn
            else:
                #turn_count gives the hovercraft to converge to a certain angle when turning
                turn_count =0
                turn_count_max=0
                
                #if both walls have an inward facing corner and they are both not too far away it is a dead end
                if((totalCornerNum >= 2) and (abs(leftData['points_2'][0]) < 200) and (abs(rightData['points_2'][0]) < 200)):
                    #turn around 180 degrees
                    
                    if(turn_count_max>0): #something has gone wrong
                        print("Fake turning!!!!")
                    else:
                        print("Turn 180 degree!!!!")
                        #turn around 180 degrees
                        headingAngle_ref = headingAngle_ref + 180 #increase the desired angle for the heading angle of the hovercraft by 180 degrees
                        steerPreviousError = 0
                    ...
                    if((leftData['cornerNum'] == 1) and (abs(leftData['points_2'][0]) < 200)): #if there is a corner on the left side and it is not too far away
                        ...
                        if(turn_count_max>0): #something has gone wrong
                            ...
                            headingAngle_count = 100
                            headingAngle_ref = headingAngle_ref - 90 #decrease the desired angle for the heading angle of the hovercraft by 90 degrees
                            turn_count_max = 50
                            steerPreviousError = 0
                    if((rightData['cornerNum'] == 1) and (abs(rightData['points_2'][0]) < 200)): #if there is a corner on the right side and it is not too far away
                        if(turn_count_max>0): #something has gone wrong
                            print("Fake turning!!!!")
                        else:
                            print("Turn 90 degree!!!!")
                            ...
                            headingAngle_count = 100
                            headingAngle_ref = headingAngle_ref + 90 #increase the desired angle for the heading angle of the hovercraft by 90 degrees
                            turn_count_max = 50
                            steerPreviousError = 0

                if(turn_count<turn_count_max):
                    while(turn_count<turn_count_max):        #give the hovercraft some time to converge to desired heading angle
                        if(abs(angle_now-headingAngle_ref)<7):
                            turn_count = turn_count +1
                        else:
                            turn_count = 0 #restart counting

                        if((abs(rightData['line_dist']) < 100) and (abs(leftData['line_dist']) < 100)): #if both walls are relatively close, maintain the center of the track
                            dist_diff = rightData['line_dist']-leftData['line_dist']
                            dist_diff = 0.9*lidarPreviousError + 0.1*dist_diff
                        else: #just pretend that both walls are the same distance away from the hovercraft
                            dist_diff = 0
                            dist_diff = 0.9*lidarPreviousError + 0.1*dist_diff
                        targetAngle= headingAngle_ref 
                        angle_now = angle_IMU
                        gyro_now = gyro_IMU
                        front_motor = front_test
                        rear_motor = rear_test
                        headingAngle_count = headingAngle_count + 1
                        if (headingAngle_count > headingAngle_count_max):
                            headingAngle_count = headingAngle_count_max
                        headingAngle = (1-(headingAngle_count/headingAngle_count_max))*angle_now + (headingAngle_count/headingAngle_count_max)*headingAngle_ref - lidarKp*dist_diff #filter
                        servo_angle = newMap(steerPidControl(headingAngle,angle_now,gyro_now),-90,90,180,0)
                        #control servos and motors directly
                        kit.servo[4].angle = rear_motor
                        kit.servo[11].angle = front_motor
                        kit.servo[15].angle = servo_angle 
                        self.rear.publish(int(headingAngle))
                        self.servo.publish(int(angle_now))
                        print("Turning!!!!")
                        print("headingAngle: " + str(headingAngle) + "  " + "angle_now: " + str(angle_now))
                        
                #if the hovercraft has converged close enough to the desired angle, go straight
                else:
                    print("Go Straight!!!!")
        ...
        #left and right are dictionaries that contain point data
